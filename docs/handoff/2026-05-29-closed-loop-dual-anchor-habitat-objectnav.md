...
- Linux pull and pressure CLI smoke after commit/push.
...
1. Push the pressure CLI/report entry point.
2. Verify it on Linux in the `habitat` conda environment.
3. Implement the first closed-loop dual-session simulator slice:
   session 1 explores and stores memory, session 2 restarts with frame drift,
   the policy chooses memory pose vs frontier, and matching evidence affects
   the decision.
