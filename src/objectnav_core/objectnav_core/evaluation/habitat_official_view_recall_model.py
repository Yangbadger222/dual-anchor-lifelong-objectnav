...
DEFAULT_FEATURE_NAMES: tuple[str, ...] = (
    ...
    "action_turn_right",
    "action_stop",
    "decision_turn_toward_memory_active_perception_frontier",
    "decision_move_toward_memory_active_perception_frontier",
    "decision_turn_toward_memory_belief_frontier",
    "decision_move_toward_memory_belief_frontier",
    "decision_fallback_occupancy_frontier",
    "decision_orient_memory_anchor_from_active_viewpoint",
    "decision_scan_memory_anchor_from_active_viewpoint",
    "decision_reacquire_detector_target",
    "decision_center_detector_target",
    "phase_none",
    "phase_orient_anchor",
    "phase_scan_anchor",
)
_SCORE_CSV_BASE_FIELDS: tuple[str, ...] = (
    "row_index",
    "episode_index",
    "episode_id",
    "scene_id",
...
def train_official_view_recall_logistic_model(
...
def predict_official_view_recall(
...
def score_official_view_recall_candidates(
    model: Mapping[str, Any],
    example: Mapping[str, Any],
    *,
    ...
    scores = {
        str(action): predict_official_view_recall(
            model,
            example,
            action=str(action),
        )
    ...
    return {
        "task": "habitat_official_view_recall_candidate_scores",
...
def score_official_view_recall_dataset(
    dataset: Mapping[str, Any],
    model: Mapping[str, Any],
    *,
    actions: Sequence[str] = ("move_forward", "turn_left", "turn_right"),
    ...
    for row_index, example in enumerate(examples):
        label_value = _label(example, label_name=label_name)
        prediction = predict_official_view_recall(model, example)
        labels.append(label_value)
        predictions.append(prediction)
        candidate_scores = score_official_view_recall_candidates(
            model,
            example,
            actions=selected_actions,
        )
        observed_action = str(example.get("action", ""))
        best_action = str(candidate_scores.get("best_action") or "")
        ...
        rows.append(
            {
                ...
                "active_perception_phase": str(
                    features.get("active_perception_phase", "")
                ),
                ...
                "candidate_scores": candidate_scores.get("scores", {}),
            }
        )
    return {
        "task": "habitat_official_view_recall_score_report",
        "label_name": label_name,
        ...
        "metrics": _metrics(labels=labels, predictions=predictions),
        "aggregate": {
            "best_action_counts": _string_counts(row["best_action"] for row in rows),
            ...
            "top_1_positive_count": _top_k_positive_count(rows, 1),
            "top_5_positive_count": _top_k_positive_count(rows, 5),
            "top_10_positive_count": _top_k_positive_count(rows, 10),
        },
        "groups": {
            "action": _group_rows(rows, "observed_action"),
            "decision": _group_rows(rows, "decision"),
            "active_perception_phase": _group_rows(rows, "active_perception_phase"),
        },
        "rows": rows,
    }


def write_official_view_recall_scores_csv(
    path: str | Path,
    ...
    *,
    ...
    fieldnames = [*_SCORE_CSV_BASE_FIELDS, *score_fields]
    ...
    with output_path.open("w", encoding="utf-8", newline="") as handle:
        ...
        for row in rows:
            ...
            payload = {
                ...
                for field in _SCORE_CSV_BASE_FIELDS
            }
...
def _top_k_positive_count(rows: Sequence[Mapping[str, Any]], k: int) -> int:
    selected = sorted(
        rows,
        key=lambda row: _to_float(row.get("prediction"), default=0.0),
        reverse=True,
    )[: max(0, int(k))]
    return sum(1 for row in selected if bool(row.get("label_value")))
...
def _group_rows(
    ...
    for key, group_rows in sorted(grouped.items()):
        positive_count = sum(1 for row in group_rows if bool(row.get("label_value")))
        prediction_sum = sum(
            _to_float(row.get("prediction"), default=0.0) for row in group_rows
        )
        summary[key] = {
            "example_count": len(group_rows),
            "positive_count": positive_count,
            "negative_count": len(group_rows) - positive_count,
            "mean_prediction": round(
                prediction_sum / float(max(1, len(group_rows))),
                6,
            ),
        }
    return summary
...
def _filter_training_examples(
    ...
    *,
    ...
    if not current_hidden_only:
        return list(examples)
    filtered = [
        example
        for example in examples
        if not _example_current_target_visible(example)
    ]
    if not filtered:
        raise ValueError("dataset has no current-hidden examples")
    return filtered
...
def _label(example: Mapping[str, Any], *, label_name: str = LABEL_NAME) -> float:
    ...
    if label_name == LABEL_NAME:
        return 1.0 if (
            not _example_current_target_visible(example)
            and bool(labels.get("target_visible_within_horizon"))
        ) else 0.0
...
def _feature_values(
    ...
    *,
    ...
    decision = str(example.get("decision", ""))
    phase = str(features.get("active_perception_phase", "")) or "none"
    anchor_bearing_error = _optional_float(features.get("anchor_bearing_error_rad"))
    values: dict[str, float | None] = {
        "distance_to_anchor_m": _optional_float(features.get("distance_to_anchor_m")),
        "anchor_bearing_error_rad": anchor_bearing_error,
        "abs_anchor_bearing_error_rad": (
            abs(anchor_bearing_error) if anchor_bearing_error is not None else None
        ),
        "expected_evidence": _optional_float(features.get("expected_evidence")),
        "view_quality": _optional_float(features.get("view_quality")),
        "path_distance_m": _optional_float(features.get("path_distance_m")),
        "travel_distance_m": _optional_float(features.get("travel_distance_m")),
        "active_perception_scan_steps_remaining": _optional_float(
            features.get("active_perception_scan_steps_remaining")
        ),
        ...
        "decision_turn_toward_memory_active_perception_frontier": _one_hot(
            decision,
            "turn_toward_memory_active_perception_frontier",
        ),
        "decision_move_toward_memory_active_perception_frontier": _one_hot(
            decision,
        ...
        "decision_move_toward_memory_belief_frontier": _one_hot(
            decision,
        ...
        "decision_orient_memory_anchor_from_active_viewpoint": _one_hot(
            decision,
            "orient_memory_anchor_from_active_viewpoint",
        ),
        "decision_scan_memory_anchor_from_active_viewpoint": _one_hot(
            decision,
            "scan_memory_anchor_from_active_viewpoint",
        ),
        ...
        "phase_none": _one_hot(phase, "none"),
        "phase_orient_anchor": _one_hot(phase, "orient_anchor"),
        "phase_scan_anchor": _one_hot(phase, "scan_anchor"),
    }
...
def _metrics(
    *,
    ...
    auc = _roc_auc(labels, predictions)
    return {
        "example_count": len(labels),
        "positive_count": positive_count,
        "negative_count": negative_count,
...
def _roc_auc(
    ...
    pairs = sorted(
        ((float(prediction), 1 if bool(label) else 0) for label, prediction in zip(labels, predictions)),
