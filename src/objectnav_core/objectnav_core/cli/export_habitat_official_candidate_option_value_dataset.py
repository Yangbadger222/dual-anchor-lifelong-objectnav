...
from objectnav_core.evaluation.habitat_official_candidate_rollout_dataset import (
    DEFAULT_OPTION_HORIZON_STEPS,
    DEFAULT_OPTION_PROGRESS_THRESHOLD_M,
    DEFAULT_OPTION_SCAN_STEPS,
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Export detector labels from official Habitat candidate option "
            "rollouts."
        )
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=DEFAULT_OPTION_HORIZON_STEPS,
        help="Short-horizon action budget for each candidate option.",
    )
    parser.add_argument(
        "--option-scan-steps",
        type=int,
        default=DEFAULT_OPTION_SCAN_STEPS,
        help="Bounded local scan budget after reaching or blocking an option.",
    )
    parser.add_argument(
        "--option-progress-threshold-m",
        type=float,
        default=DEFAULT_OPTION_PROGRESS_THRESHOLD_M,
        help=(
            "Minimum official distance-to-goal improvement required for the "
    ...
    parser.add_argument(
        ...
        default="none",
        help="Detector used for candidate option-value labels.",
    )
    parser.add_argument(
        "--detector-weights",
        default=None,
...
def main(
    ...
    *,
    ...
    run = runner or export_official_candidate_option_value_dataset
    dataset = run(
        args.policy_trace,
        ...
        option_horizon_steps=args.option_horizon_steps,
        option_scan_steps=args.option_scan_steps,
        option_progress_threshold_m=args.option_progress_threshold_m,
...
def _summary(dataset: Mapping[str, Any]) -> dict[str, Any]:
    return {
        ...
        "candidate_option_count": dataset.get("candidate_option_count"),
        "positive_option_count": dataset.get("positive_option_count"),
        "invalid_option_count": dataset.get("invalid_option_count"),
    }
